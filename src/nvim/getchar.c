...
// otherwise, returns the character that should be returned by vgetorpeek().
//
                           ...
                           const int local_State)
{
  ...
  if (mp == NULL && *keylenp != KEYLEN_PART_MAP) {
...
static int ins_esc_special_case(int *new_wcolp, int *new_wrowp,
                                bool *mode_deletedp,
...
static int vgetorpeek(const int advance)
{
  ...
  //     message is deleted.
  //     If it's not going to return an ESC but has already deleted the
  //     message, redisplay it.
  //     This can be the case if we waited on an insert-mode mapping that began
  //     with ESC, and cleared the mode to give the appearance of an instant
  //     reaction to the ESC.
